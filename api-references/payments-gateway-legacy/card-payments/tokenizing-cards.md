...
There as two ways of interacting with the tokenization, and over the next months, native SDKs to the iOS and Android platform will also be available.

1. Tokenizer component that takes over an existing form and requires the additional of a few javascript lines to interact, but provides greater customization;
2. Renderable code that injects the card components on screen directly;

**Example 1: adding a tokenizer component to an existing form.**
...
| sandbox         | true or false           | Whether using sandbox or production.                                                                         |
...
**Example 2: Rendering the card component, using our standard library**

Using this component you will be able to save some time by simply rendering our javascript component in an existing page.
...
<head>
  ...
  <title>Example 01 - Basic Example</title>
	<script src="<https://cdn.simpleps.com/sandbox/inyo.js>"></script>
...
<body>
	  ...
	  <div style="margin-top: 50px">
	    <form id="f1" method="POST" action="">
	        <div id="card1" class="card-js" data-capture-name="true"></div>
	        <button type="submit"  class="btn" id="_btn1">Validate</button>
	    </form>
	  </div>
	  ...
	  <script>
	    var a1 = new InyoForm({
	        ...
	        successCallback: function(data){
	            ...
	            // send the token to your backend in order to authorize the transaction
	
	            var token = data.additionalData.token;
	            var amount = 100;
